...
def connected_count(matrix, pos):
    ...
    while unexplored:
        x, y = unexplored.pop()
        explored.append([x, y])
        for x_, y_ in [((x + 1) % height, y),  # down
                       ((x - 1) % height, y),  # up
                       (x, (y + 1) % width),  # right
                       (x, (y - 1) % width)]:  # left
            if matrix[x_, y_] == sign and [x_, y_] not in explored and [x_, y_] not in unexplored:
    ...
    def claim2action(self, claim_position, snake_idx, step_count, output_type):
        ...
        while step_count > 1:
            ...
            for x_, y_ in [((x + 1) % self.height, y),  # down
                ...
                state = sign // self.snakes_count if sign > 0 else -(sign // self.snakes_count)
                indexes = [sign % self.snakes_count] if sign >= 0 else self.controversy[(x_, y_)]['indexes']
...
def state2claims(state_array, max_state, priority):
    ...
    horiz = [list(state_array[_]).count(max_state) for _ in range(beanCount)]  # [5, 5, 4, 4, 5]
    vert = [list(state_array[:, _]).count(max_state) for _ in range(snakeCount)]  # [4, 4, 4, 3, 4, 4]
    claim_order = []
...
def my_controller(observation_list, action_space_list=None, is_act_continuous=None):
    with HiddenPrints():
        ...
        assert teams is not None, 'unknown game with observation length {}'.format(observation_len)
        ...
        defense_snakes_indexes = []  # save defensive or claimed snakes, to calculate safe move for ctrl snake
        ...
        defense_threshold = board_height * board_width * teams_count / (teams_count + 1) / snakes_count
        ...
        for idx in team_indexes:
            if snakes_lens[idx] > defense_threshold:
                ...
                for _ in range(1, min(bd.state, snakes_lens[idx] // 2)):
                    ...
                    if bd.board[x, y] == idx + _ * snakes_count:  # claim a loop in step _
                        defense_snakes_indexes.append(idx)
                        defensive_claim_list.append([[x, y], idx, _])
                        if idx == ctrl_agent_index:
                            action = [bd.claim2action(claim_position=[x, y],
                                                      snake_idx=idx,
                                                      step_count=_,
                                                      output_type='action')]
                            print('the controlled agent {} make a defensive move {} within {} step(s)'
                                  .format(idx, action[0], _))
                            print(
                                '***********************************' +
                                ' defensive move ' +
                                '***************************************')
        ...
        state_array = np.zeros((len(beans_positions), len(snakes)), dtype=int) + max_state
        for i, (x, y) in enumerate(beans_positions):
            ...
            elif sign < 0 and sign % snakes_count in [_ for _ in range(snakes_count) if _ > 0]:  # [2, 3, 4, 5]
        ...
        claim_list = claim_list_byState if len(claim_list_byState) >= len(claim_list_byCover) else claim_list_byCover
        ...
        for c in claim_list:
            if ctrl_agent_index == c[1]:  # the controlled agent claim a bean
                print('the controlled agent {} claim a bean {} within {} step(s)'.format(c[1], c[0], c[2]))
                action = [bd.claim2action(claim_position=bd.beans_positions[c[0]],
                                          snake_idx=c[1],
                                          step_count=c[2],
                                          output_type='action')]
            ...
            free_team_snakes_indexes = [_ for _ in team_indexes
            ...
            for idx in free_team_snakes_indexes:
                ...
                for x, y in [((x_h + 1) % bd.height, y_h),  # down
                             ((x_h - 1) % bd.height, y_h),  # up
                             (x_h, (y_h + 1) % bd.width),  # right
                             (x_h, (y_h - 1) % bd.width)]:  # left
                    if bd.board[x][y] == idx + snakes_count:
                        safe_positions.append([x, y])  # should be further tested if exists breath!!
            ...
            free_team_snakes_indexes = list(np.array(free_team_snakes_indexes)[check_list])  # [idx1, idx2]
            safe_positions_list = [_ for _ in safe_positions_list if _]  # [[pos1, pos2, pos3], [pos1, pos2, pos3]
            ...
            for c in claim_list:  # claimed snake make one move
                ...
                position = bd.claim2action(claim_position=bd.beans_positions[c[0]],
                                           snake_idx=idx,
                                           step_count=c[2],
            ...
            for c in defensive_claim_list:  # claimed snake make one move
                idx = c[1]
                position = bd.claim2action(claim_position=c[0],
                                           snake_idx=idx,
                                           step_count=c[2],
                                           output_type='position')
                snakes_next[idx].step(position)
            ...
            for pos_comb in itertools.product(
                ...
                for i, idx in enumerate(free_team_snakes_indexes):  # unclaimed and undead snake make one move
                ...
                bd_next = Board(board_height, board_width, snakes_next, beans_positions, teams)
                with HiddenPrints():
                    while not all(
                            _ == [] for _ in bd_next.open.values()):  # loop until all values in open are empty list
                        bd_next.step()
                print(bd.board)
                claimed_counts = np.zeros(len(team_indexes))
                for i, idx in enumerate(team_indexes):  # not free, consider all team snakes!!
            ...
            action[0][direction] = 1
            print('the controlled agent {} play a random action and is dying'.format(ctrl_agent_index), action[0])
            print('*********************************** random move ******************************************')
            return action
